...
internal sound
LoadSoundFromWavData(memory_block *Block, void *WavData, u64 WavDataSize)
{
    sound Result          = {};
        ...
        if(RIFFTest && WAVETest)
        {
            ...
            if(FormatFound && DataFound)
            {
                ...
                if((u64)(DataPtr - (u8*)WavData) + DataSize <= WavDataSize)
                {
                    ...
                    Result.Samples = (float*)PushSize(Block, SampleAllocSize);
    ...
    {
        ZeroSize(sizeof(Result), (void*)&Result);
        return Result;
    }
}

internal sound
LoadSoundFromWavFile(memory_block *Block, char *File)
{
    read_file_result WavFile = Platform.ReadEntireFile(File);
    sound Result = {};
    if(WavFile.Contents)
    {
        Result = LoadSoundFromWavData(Block, WavFile.Contents, WavFile.ContentsSize);
