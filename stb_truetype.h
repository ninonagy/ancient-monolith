      ...
      if (e->fdx == 0) {
         ...
         if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
            ...
            if ((int) x_top == (int) x_bottom) {
               ...
               int x,x1,x2;
               float y_crossing, step, sign, Chunk;
               // covers 2+ pixels
               if (x_top > x_bottom) {
                  // flip scanline vertically; signed Chunk is the same
                  float t;
               ...
               sign = e->direction;
               // Chunk of the rectangle covered from y0..y_crossing
               Chunk = sign * (y_crossing-sy0);
               // Chunk of the triangle (x_top,y0), (x+1,y0), (x+1,y_crossing)
               scanline[x1] += Chunk * (1-((x_top - x1)+(x1+1-x1))/2);

               step = sign * dy;
               for (x = x1+1; x < x2; ++x) {
                  scanline[x] += Chunk + step/2;
                  Chunk += step;
               }
               y_crossing += dy * (x2 - (x1+1));

               STBTT_assert(STBTT_fabs(Chunk) <= 1.01f);

               scanline[x2] += Chunk + sign * (1-((x2-x2)+(x_bottom-x2))/2) * (sy1-y_crossing);
